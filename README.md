...
| 유형 | 판정 방식 |
|---|---|
| 📊 지표 예측 | **완전 자동.** 매일 수집되는 지표값 + 실시간 시세로 판정 — 기한 내 목표에 1회라도 도달하면 적중, 기한이 지나면 빗나감 |
| ✏️ 자유 예측 | **AI 자동 판정(선택).** 예측 노트의 '🤖 AI 자동 판정'에 Anthropic API 키를 등록하면, 기한이 된 예측을 기간 중 수집된 뉴스 헤드라인(`data/news-archive.json`, 최근 60일)을 근거로 Claude가 판정 |

AI 판정 동작 방식:
- 브라우저에서 api.anthropic.com을 직접 호출합니다. 키는 localStorage에만 저장되며 이 저장소나 서버로 전송되지 않습니다
- 증거가 불충분하면 **보류**하고 직접 판정을 기다립니다 (틀린 자동 판정보다 보류가 낫다는 원칙)
- 자동/AI 판정된 결과는 '정정' 버튼으로 뒤집을 수 있습니다
- API 키는 [Anthropic Console](https://platform.claude.com/)에서 발급. 판정 1건당 소액(수십 원 수준)의 사용료가 발생합니다
